...
VENV_DIR="${PROJECT_DIR}/src/layers/bronze/venv/"
...
aws s3 cp "$ZIP_FILE" "s3://$S3_BUCKET/scrapers/lambdas/$ZIP_FILE"

# Deploy the update to AWS Lambda using S3
echo "Updating AWS Lambda function to use the uploaded S3 object..."
echo "Bucket: $S3_BUCKET"
echo "Key: scrapers/lambdas/$ZIP_FILE"
aws lambda update-function-code --function-name $FUNCTION_NAME --s3-bucket $S3_BUCKET --s3-key "scrapers/lambdas/$ZIP_FILE"
